...
def userHandler():
    ...
    while userInput != "q":
        print 
        userInput=raw_input('Podaj szukane słowo lub frazę:')
        userInputParsed=usunNoweLinie(usunZnakiPrzestankowe(userInput.lower()))
        result = DB_HANDLER.execute('SELECT ID, LINK, CONTENT from PAGES WHERE LINK LIKE "%'+userInput+'%" OR CONTENT LIKE "%'+userInputParsed+'%" LIMIT 5')
        print "Lista stron dokładnych (max 5)"
        idPierwszej=-1
        for row in result:
            if idPierwszej==-1:
                idPierwszej = row[0]
            print " ID: "+str(row[0])+" LINK: "+row[1]
            print u"  TREŚĆ: "+row[2][:40]
        DB_HANDLER.execute('SELECT count(*) from PAGES WHERE LINK LIKE "%'+userInput+'%" OR CONTENT LIKE "%'+userInputParsed+'%" LIMIT 5')
        count=DB_HANDLER.fetchone()
        print "Słowo/fraza wystąpiła w " +str(count[0])+ " tekstach."
        if (count[0] > 0):
            znajdzStronyPodobne(idPierwszej)
